$(document).ready(function(){        
    ...
    vendor_menu_counter = 3; //by defult 3 inputs

        $('#addlink').click(function(e) {
        e.preventDefault();
        vendor_menu_counter++;
        $('#vendor_menu_counter').val(vendor_menu_counter);
     
        if(vendor_menu_counter > 3) {
            $('#removelink').show();
        }else{
            $('#removelink').hide();
        }
        var html = '';
        html  +='<div class="form-group" id="menu_'+vendor_menu_counter+'">';
        html +='<label class="col-md-3 col-xs-12 control-label">Item  Name</label>';
        
        html += '<div class="col-md-2" id="item2_'+vendor_menu_counter+'">';
        html += '<input type="text" class="form-control" name="item_name_'+vendor_menu_counter+'" />';
        html += '</div>';

        html +='<label class="col-md-2 col-xs-3 control-label">price per plate</label>';
         html += '<div class="col-md-2" id="item_price2_'+vendor_menu_counter+'">';
        html += '<input type="text" class="form-control" name="item_price_'+vendor_menu_counter+'" />';
        html += '</div>';

        html +='</div>'
        
        $('#myform').append(html).fadeIn(4500);
        
        });

    $('#removelink').click(function(e) {
        e.preventDefault();
        $('#menu_'+vendor_menu_counter).remove();
        vendor_menu_counter--;
    ...
    $('.apprv').click(function() {
        ...
        if(check) {
            ...
            var approve_url = '';
            approve_url += base_url+'events/api_change_status';
            ...
            $.ajax({
                ...
                success : function(resp) {
                    window.location.href = base_url+"events/view-all";
                }
            });
    ...
    $('.sponsor').click(function() {
        ...
        if(check) {
            ...
            var sponsor_url = '';
            sponsor_url += base_url+'events/api_make_sposnored';
	...
	$('#state_id').change(function() {
		...
		if(state_id > 0 )
		{
			...
			url_to_call = '';
			url_to_call += base_url+'cities/api_get_cities';
...
//prepare city dropdown list
function render_city_ui(resp) {
	var html = '';
